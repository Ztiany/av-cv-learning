...
import me.ztiany.androidav.utils.AppDirectory;
import me.ztiany.androidav.utils.YUVUtils;
...
public class Camera1BackSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private void startPreview() {
        ...
        try {
            ...
            mCamera.setPreviewCallbackWithBuffer((bytes, camera) -> {
                ...
                if (isCapture) {
                    isCapture = false;
                    capture(YUVUtils.rotateNV21(bytes, size.width, size.height, 90));
                }
    ...
    public void capture(byte[] temp) {
        //保存一张照片
        File pictureFile = new File(AppDirectory.createDCIMPicturePath(AppDirectory.PICTURE_FORMAT_JPEG));
