...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
	...
	*/
	...
	float lightPos[] = { 0.0f,1.0f,0.0f,0.0f };//方向光
	glLightfv(GL_LIGHT0, GL_AMBIENT, whiteColor);
	glLightfv(GL_LIGHT0, GL_DIFFUSE, whiteColor);
	glLightfv(GL_LIGHT0, GL_SPECULAR, whiteColor);
	glLightfv(GL_LIGHT0, GL_POSITION, lightPos);//设置方向光. 除此之外还有 point,spot 光。

	//材质
	float blackMat[] = { 0.0F,0.0F ,0.0F ,1.0F };
	float ambientMat[] = { 0.1f,0.1f,0.1f,1.0f };//环境光
	float diffuseMat[] = { 0.4f,0.4f,0.4f,1.0f };//漫反射
	float specularMat[] = { 0.9f,0.9f,0.9f,1.0f };//镜反射
	...
	while (true) {//这个 while 的执行跟刷新率有关系，比如 60hz 的刷新率每秒跑 60 次。
		...
		glNormal3f(0.0f, 1.0f, 0.0f);//法线，与漫反射有关。
