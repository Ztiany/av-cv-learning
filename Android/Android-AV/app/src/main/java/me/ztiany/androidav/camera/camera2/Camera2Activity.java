...
public class Camera2Activity extends AppCompatActivity implements ViewTreeObserver.OnGlobalLayoutListener, Camera2Listener {
    ...
    public void onCameraOpened(CameraDevice cameraDevice, String cameraId, final Size previewSize, final int displayOrientation, boolean isMirror) {
        ...
        runOnUiThread(() -> {
            ivPreviewFrame = new BorderImageView(Camera2Activity.this);
            ivOriginFrame = new BorderImageView(Camera2Activity.this);
            TextView tvPreview = new TextView(Camera2Activity.this);
            TextView tvOrigin = new TextView(Camera2Activity.this);
        ...
        if (currentIndex++ % PROCESS_INTERVAL == 0) {
            imageProcessExecutor.execute(() -> {
                ...
                if (y.length / u.length == 2) {
                    YUVUtils.yuv422ToYuv420sp(y, u, v, nv21, stride, previewSize.getHeight());
                }
                // 回传数据是YUV420
                else if (y.length / u.length == 4) {
                    YUVUtils.yuv420ToYuv420sp(y, u, v, nv21, stride, previewSize.getHeight());
                }
